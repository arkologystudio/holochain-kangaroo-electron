...
const HAPP_FILE = "../pouch/data-commons-mvp.happ"; // replace-me Enter the path to your happ
const APP_ID = "data-commons-mvp"; // replace-me
const LAIR_PASSWORD = "password";
const WINDOW_TITLE = "data-commons-mvp".toUpperCase(); //replace-me
...
export function stateSignalToText(state: StateSignal): string {
  switch (state) {
    case StateSignal.IsFirstRun:
      return "Welcome to data-commons-mvp...";
...
const createSplashWindow = (): BrowserWindow => {
  ...
  if (SPLASHSCREEN_WINDOW_VITE_DEV_SERVER_URL) {
    ...
    splashWindow.loadFile(
      path.join(
        __dirname,
        `../renderer/${SPLASHSCREEN_WINDOW_VITE_NAME}/index.html`
      )
    );
  }
...
app.on("ready", async () => {
  ...
  statusEmitter.on(LAIR_SOCKET_EVENT, async (lair_socket_url: string) => {
    ZOME_CALL_SIGNER = await ZomeCallSigner.connect(
      lair_socket_url,
      LAIR_PASSWORD
    );
  });
